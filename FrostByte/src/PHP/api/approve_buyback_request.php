...
try {
    ...
    if (!isset($input['request_id']) || !isset($input['offer_price'])) {
        throw new Exception("Missing required fields");
    }

    $request_id = (int)$input['request_id'];
    $offer_price = (float)$input['offer_price'];
    $approval_message = $input['offer_message'] ?? '';
    ...
    $getQuery = "SELECT * FROM sell_parts_requests WHERE id = ?";
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Sell part request approved',
        'data' => $updatedRequest
    ]);
